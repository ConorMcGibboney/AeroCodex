...
This document records the current BioSim and Orekit v3 serial plan after the deployed O2a, adapted Session E, and adapted Session F slices. It is documentation/status control only. It does not deploy runtime/code, apply any patch, approve source-locator normalization, import external source, or change governed inventories.

## Summary

- Orekit v3 O2a time/frame/Cartesian-state foundation is deployed/completed at `2f1e64ea7638b2f54071eca488c26252256235ca`.
- Orekit v3 O2b classical-elements/Kepler numerical-policy review and bounded deployment is the next recommended implementation candidate and remains `needs_review`.
- Orekit O2c remains `blocked` by O2b; Orekit O2d remains `blocked` by O2c.
- Adapted Session F reference-oracle planning metadata is deployed at `68dc10fc9215df2be9bc64e0f2a94121250c361a`, but it is not a replacement for O2b, O2c, or O2d.
...
| 2 | O2b classical elements, Kepler helpers, deterministic smoke example | next bounded candidate | needs_review | Depends on O2a. | Requires separate authorization, numerical-policy review, source-boundary review, live count-delta computation, full gates, and exact deployed CI. |
| 3 | O2c oracle records and tolerance comparison helpers | blocked_until_predecessor | blocked | Depends on O2b. | Metadata-only oracle evidence; no external fixtures copied. |
| 4 | O2d two-line element contract/source policy | blocked_until_predecessor | blocked | Depends on O2c. | Contract/source policy only; no parser or propagator/frame implementation. |
...
| Adapted Session F Orekit reference-oracle planning metadata | Deployed as planning metadata only; Orekit O2b/O2c/O2d remain open. | superseded |
...
| Older Orekit v1/v2 materials, O1a, and companion notes | Audit inputs only; deployed O2a and remaining O2b/O2c/O2d define the Orekit lane. | superseded |
...
- Orekit O2b numerical-policy review before classical-elements/Kepler deployment.
- Orekit O2c and O2d predecessor closure.
