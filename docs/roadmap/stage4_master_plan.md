...
Chunk 6B adds a clean-room one-tick atomic resource-delta commit primitive in `crates/aero-codex-life-support`. Chunk 6C adds deterministic ordering of caller-supplied resource states and deltas plus dependency-free fnv-1a before/after digest evidence for one transaction. Chunk 6D adds grouped resource-ledger residual checks and a bounded two-store oxygen-loop conservation proof. All remain `research_required`, import no GPL source or scaffold crates, and make no persistent-ledger-storage, scenario-execution, habitat-safety, medical, operational, or certification claim.
...
| 6B | BioSim-RS atomic transaction commit | Add a clean-room one-tick resource-delta commit helper, source seed, validation card, and assurance doc after Chunk 6A closes. | Public life-support primitive only; no GPL source import, scaffold crate import, replay proof, persistent ledger, conservation model, or readiness claim. |
| 6C | BioSim-RS deterministic ordering, digest, and replay proof | Add clean-room canonical resource ordering, fnv-1a state/delta digests, one-tick replay proof helper, source seed, validation card, and assurance doc after Chunk 6B closes. | Public life-support primitive only; no GPL source import, scaffold crate import, persistent ledger, conservation model, scenario execution, or readiness claim. |
| 6D | BioSim-RS resource ledger and minimal oxygen-loop conservation | Add clean-room grouped resource-ledger residual checks and a bounded two-store oxygen transfer conservation proof after Chunk 6C closes. | Public life-support primitive only; no GPL source import, scaffold crate import, persistent ledger storage, scenario execution, biological dynamics, habitat-control behavior, or readiness claim. |
| 6E | BioSim-RS CLI/API smoke tests and friend-test report | Add static clean-room API smoke and package-example report over the completed Chunk 6A through Chunk 6D primitives. | Public life-support smoke/report helper only; no GPL source import, scaffold crate import, external fixtures, scenario execution, persistent command surface, habitat-control behavior, or readiness claim. |
| 7A | Formula-vault implementation-candidate gate | Add `docs/assurance/formula_vault_candidate_gate.md`, a non-operative `formula-vault/templates/implementation_candidate_slice.yaml`, and matching validation/source-registry records for future per-slice candidate gates. | Metadata/governance only; no M07 source-code import, formula implementation, public API, Scilab execution, SGP4 claim, or readiness claim. |
| 7B | Formula-vault first metadata-only candidate slice | Add bounded M00 angle/unit conversion metadata under `formula-vault/candidates/m00_angle_unit_conversions.yaml`, assurance note, validation card, and source-registry seed for three release-gate rows only. | Metadata only; no M07 source-code import, formula implementation, public API, Scilab execution result, fixture import, SGP4 claim, or readiness claim. |
| 7C | Formula-vault candidate verifier skeleton | Add dependency-free `cargo run -p xtask -- verify formula-vault`, negative-case tests for missing fields and duplicate identifiers, and verifier/source validation metadata. | Metadata/governance verifier only; no M07 source-code import, formula implementation, public application programming interface, Scilab execution result, SGP4 claim, fixture import, or readiness claim. |
| 7D | Formula-vault per-candidate manifest/reference-link depth | Add a metadata-only manifest for the M00 angle/unit candidate, cross-link candidate/assurance/validation/source/verifier/intake records, and deepen row/function/source-file reference status without importing source expressions. | Metadata/governance only; no M07 source-code import, formula implementation, public application programming interface, Scilab execution result, fixture import, SGP4 claim, or readiness claim. |
| 7E | Equation inventory/readiness dashboard | Add `validation/equation_inventory.tsv`, assurance/validation/source records, and dependency-free `cargo run -p xtask -- verify equation-inventory` coverage for executable research equations, metadata-only candidates, external M07 backlog rows, validation-card-only records, source-registry seeds, helper algorithms, and row-local block reasons. | Metadata/governance verifier only; no formula implementation, M07 code import, generated formula code, public application programming interface, Scilab execution result, fixture import, or readiness claim. |
| 7F | M00 source-expression and test-vector contract | Add `formula-vault/contracts/m00_angle_unit_conversions_contract.yaml`, assurance/validation/source records, and equation-inventory updates for the three selected M00 candidates. | Metadata/test-contract only; no formula implementation, M07 source import, generated formula code, public application programming interface, Scilab execution result, fixture import, or readiness claim. |
| 8A | M00 vector-equation expansion | Add a bounded M00 vector-algebra candidate slice, independent contract, validation/source records, and public Rust research kernels for deg/rad conversions plus finite 3-vector helpers. | Research kernels only; no raw M07 source import, generated code, Scilab output, fixture import, application-interface promotion, SGP4 claim, or readiness claim. |
| 7+ | Formula-vault and implementation candidates | Continue only after formula-vault gates, per-slice source/validation records, and manifest/reference-link records exist. | Bounded by per-slice gates. |
...
- validation status that distinguishes static provenance, unit tests, reference-oracle tests, source-equivalence tests, and explicit non-certification boundaries;
