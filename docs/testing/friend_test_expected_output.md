...
| `python scripts/verify_governance.py --repo .` | exits successfully | Formula-vault runtime resolutions, A11 external unit-conversion dispositions, and the configured xtask governance verifiers completed as a group. |
...
| External M07 rows with terminal dispositions | 38 | A11 records 37 deduplicated aliases and 1 contract-blocked row. |
...
- the A11 external unit-conversion verifier reports 38 terminal rows, 37 deduplicated aliases, 1 contract-blocked angular-rate row, and a remaining backlog of 1,285;
