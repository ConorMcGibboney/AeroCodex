...
Chunk 6A and Chunk 6B clean-room primitives are implemented in the existing life-support crate, not in this placeholder directory:
...
Future code in this placeholder directory may only be added after a later chunk records source IDs, license path, validation status, deterministic replay evidence, resource-ledger expectations, and the research/preliminary-design caveat. Chunk 6A and Chunk 6B do not authorize adding crates or external-source material under `biosim-rs/`.
