...
This README reflects current `main` after compiler-batch coverage was completed for all 152 existing Rust equation runtimes, the 27 formula-vault intake records were linked to governed runtimes, and A11-A29 assigned terminal dispositions to eighteen bounded external-backlog waves. The live governed counters remain verifier-derived; these waves change no runtime kernel count and leave every linked formula at `research_required`:
...
| External M07 rows with terminal dispositions | 703 | A11-A29: 127 deduplicated aliases, 103 excluded internal/composite helpers, and 393 contract- or policy-blocked rows. |
| External M07 backlog rows | 620 | Registered external M07 represented rows that still lack a terminal disposition. |
...
A28-A29 continue the governed solver / numerical propagation policy backlog with 80 contract- or policy-blocked rows and leave 43 rows in that candidate pool.
