...
This README reflects current `main` at the A45 repair / A46 source-snapshot state after compiler-batch coverage was completed for all 152 existing Rust equation runtimes, the 27 formula-vault intake records were linked to governed runtimes, and A11-A45 assigned terminal metadata dispositions to all tracked external M07 rows. The live governed counters remain verifier-derived; these waves change no runtime kernel count, leave every linked formula at `research_required`, and keep every external M07 row blocked as evidence-only metadata:
...
| External M07 rows with terminal dispositions | 1323 | A11-A45 metadata-only terminal dispositions across 35 bounded `formula-vault/resolutions/m07_*.tsv` manifests. These rows remain `research_required` and blocked; they do not import M07/Scilab source or add runtime kernels. |
| External M07 backlog rows | 0 | `validation/equation_inventory.tsv` records `stage4.m07.remaining_backlog_rows` as `external_m07_backlog_row` with `row_count=0`; this is evidence of backlog accounting closure, not external parity or readiness promotion. |
...
A10 runtime-resolution status: `linked_to_existing_runtime=27`, `unresolved=0`, with 3 angle/unit, 14 vector-algebra, and 10 canonical-unit links in `formula-vault/resolutions/m00_runtime_links.tsv`. A11-A45 external status: `terminal_dispositions=1323`, `remaining_backlog=0`, across 35 bounded `formula-vault/resolutions/m07_*.tsv` manifests. The external M07 dispositions are terminal metadata aliases, exclusions, contract blocks, or policy blocks only; they preserve `research_required` / blocked semantics and claim no M07/Scilab parity, certification, flight readiness, mission readiness, operational approval, or regulated-use approval.
