#!/usr/bin/env python3
"""Verify A25 orbital-geometry/conic Wave 10 terminal dispositions.

This dependency-free verifier consumes classifier metadata, A16-A24 manifests,
existing external resolution manifests, and explicit A25 terminal resolution records.
...
EXPECTED_PRIOR_ROWS=360
EXPECTED_ROWS=17
EXPECTED_GROUP_REMAINING_ROWS=0
EXPECTED_EXECUTABLE_ROWS=152
EXPECTED_METADATA_ROWS=27
EXPECTED_CUMULATIVE_PROCESSED=1170
EXPECTED_REMAINING_BACKLOG=153
EXPECTED_RISK_COUNTS=Counter({'medium_risk_requires_contract_review':14,'high_risk_requires_numerical_policy':3})
EXPECTED_DISPOSITIONS=Counter({ 'blocked_until_orbit_geometry_conic_branch_and_validation_policy':17 })
...
BLOCK_REASON='Classifier row remains blocked until orbit-geometry/conic branch conventions, frame/unit contracts, numerical policy, source registry, and independent validation oracle are explicitly approved; no runtime alias or implementation claim is made in A25.'
...
def verify_repo(repo:Path)->dict[str,Any]:
    ...
    return {'schema_version':SCHEMA_VERSION,'result':'PASS','wave_id':'a25_external_m07_orbital_geometry_conic_wave10','prior_group_rows':len(prior),'classifier_rows_selected':len(selected),'classifier_group_remaining_rows':len(after),'terminal_disposition_rows':len(rows),'deduplicated_alias_rows':0,'excluded_helper_rows':0,'contract_blocked_rows':17,'risk_tier_counts':dict(sorted(risk.items())),'target_formula_counts':{},'distinct_source_files':len(source_files),'risk_tier_not_downgraded':True,'executable_research_equations':len(executable),'metadata_inventory_records':len(metadata),'external_m07_processed_rows':total,'external_m07_backlog_rows':backlog,'formula_count_delta':0,'runtime_kernel_files_changed':0,'new_validation_cards_required':0,'new_source_seeds_required':0,'validation_status':'research_required','no_rust_m07_or_scilab_source_scraping':True,'no_external_parity_claim':True,'no_certification_or_operational_readiness_claim':True}
VALIDATOR_NAME='verify_external_m07_orbital_geometry_conic_wave10'
SELECTED_ROW_RANGE={'first':'PORT_STATUS_RELEASE_GATE.csv:row_0789','last':'PORT_STATUS_RELEASE_GATE.csv:row_0805','count':EXPECTED_ROWS}
def validation_contract_fields(errors:list[str]|None=None)->dict[str,Any]:
    return {'validator':VALIDATOR_NAME,'wave_identifier':'a25_external_m07_orbital_geometry_conic_wave10','selected_row_range':SELECTED_ROW_RANGE,'selected_row_count':EXPECTED_ROWS,'processed_backlog_counters':{'external_m07_processed_rows':EXPECTED_CUMULATIVE_PROCESSED,'external_m07_backlog_rows':EXPECTED_REMAINING_BACKLOG},'validation_checks_summary':{'supports_self_test':True,'supports_repo_argument':True,'dependency_free_python':True,'mutates_repository_files':False,'selected_rows':'row_0789 through row_0805','deduplicated_alias_rows':0,'excluded_helper_rows':0,'contract_blocked_rows':17,'risk_tier_counts':dict(sorted(EXPECTED_RISK_COUNTS.items()))},'errors':errors or []}
def self_test()->dict[str,Any]:
    errors=[]
    if EXPECTED_ROWS!=17:errors.append('EXPECTED_ROWS mismatch')
    if EXPECTED_CUMULATIVE_PROCESSED!=1170:errors.append('EXPECTED_CUMULATIVE_PROCESSED mismatch')
    if EXPECTED_REMAINING_BACKLOG!=153:errors.append('EXPECTED_REMAINING_BACKLOG mismatch')
